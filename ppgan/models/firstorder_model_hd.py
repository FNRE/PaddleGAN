...
@MODELS.register()
class FirstOrderModelHD(BaseModel):
    ...
    def __init__(self,
                 ...
                 generator,
                 discriminator=None):
        super(FirstOrderModelHD, self).__init__()
    ...
    def setup_optimizers(self, lr_cfg, optimizer):
        ...
        """
        path = "/home/paddle/PaddleGAN/data/first_order/checkpoints/paddle-vox-512-cpk.pdparams"
        weights = paddle.load(path)
        self.nets['kp_detector'].set_state_dict(weights['kp_detector'])
        self.nets['generator'].set_state_dict(weights['generator'])
        self.nets['discriminator'].set_state_dict(weights['discriminator'])
    ...
    def forward(self):
        ...
        save_model = (not self.use_parallel) or (
                    self.use_parallel and
                    paddle.distributed.get_rank() == 0)
        if (self.step % 30000 == 0 and save_model):
